...
const OUT_RUN_MANIFEST_JSON = outPath(
  options.filenames.runManifestJson,
  ".json",
);
...
const RX_FUNCTION =
  /\bfunction\b|\b\w+\s*=\s*\(?[^=]*\)?\s*=>|\b\w+\s*\([^=]*\)\s*{/g;
...
async function collectConfigSnapshots() {
  ...
  for (const relPath of candidates) {
    ...
    const { text, truncated } = await safeReadText(
      abs,
...
async function generateLintDebtContent(fileMetas, prevState) {
  ...
  for (const f of fileMetas) {
    ...
    let text;
    try {
      const { text: t } = await safeReadText(
        abs,
  ...
  const sortedFiles = [...debtByFile.entries()]
    .map(([filePath, counts]) => ({
      ...
      ...counts,
      total:
        counts.eslintDisable +
        counts.tsIgnore +
        counts.anyUsage +
        counts.todoFixmeHack,
    }))
  ...
  const topAny = [...sortedFiles]
    .sort((a, b) => b.anyUsage - a.anyUsage)
    .slice(0, topN);
...
async function generateTestMatrixContent() {
  ...
  lines.push("");
  lines.push(
    "| Component | *.spec.ts | *.stories.ts | index.ts | *.types.ts |",
  );
  lines.push("| --- | --- | --- | --- | --- |");
...
async function generateSecretsScanContent(fileMetas) {
  ...
  outputLines.push(
    "> No secret values are output. Review each finding manually.",
  );
  outputLines.push("");
...
async function writeProjectDump(fileMetas, mode, duplicatesRemoved = 0) {
  ...
  for (const f of fileMetas) {
    ...
    if (
      typeof f.textBytes === "number" &&
      f.textBytes > MAX_TEXT_BYTES_PER_FILE_DUMP
    ) {
...
async function writeStructure(mode, filesAbs, duplicatesRemoved = 0) {
  ...
  lines.push(
    `Discovery mode: ${mode === "git" ? "git ls-files" : "filesystem walk"}`,
  );
...
async function main() {
  ...
  insightsSections.push(
    `- **Excluded patterns:** ${EXCLUDED_PATTERNS_SUMMARY}`,
  );
  if (duplicatesRemoved > 0) {
  ...
  const lintDebtResult = await generateLintDebtContent(
    analysis.fileMetas,
    prevState,
  );
  console.info(
  ...
  const importGraphResult = await generateImportGraphContent(
    analysis.fileMetas,
  );
  console.info(
  ...
  const secretsScanResult = await generateSecretsScanContent(
    analysis.fileMetas,
  );
  ...
  if (CREATE_ZIP_ARTIFACT) {
    ...
    if (zipResult) {
      ...
      if (zipResult.latestZipPath) {
        console.info(
          `   Latest: ${path.relative(PROJECT_ROOT, zipResult.latestZipPath)}`,
        );
      }

 ...
 */
export function getSlotIdsForArray(
  arr: unknown[],
): readonly string[] | undefined {
  // Get the original array if arr is a proxy
  const originalArr = isProxy(arr)
    ? (getOriginalObject(arr) as unknown[])
    : arr;
  ...
  const updateSlotSidecarForArrayMutation = (
    ...
    switch (method) {
      case "push": {
        ...
        const newSlotIds = Array.from({ length: args.length }, () =>
          generateSlotId(),
        );
      ...
      get: (target, property, receiver) => {
        ...
        if (Array.isArray(target) && typeof property === "string") {
          ...
          if (arrayMutatingMethods.includes(property)) {
            const originalMethod = (target as unknown[])[
              property as keyof unknown[]
            ];

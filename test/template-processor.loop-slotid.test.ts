...
describe('template-processor loop slotId with model sidecar', () => {
  ...
  it('positive: loop over reactive array attaches slotIds from model sidecar', async () => {
    ...
    const nodes = Array.from(container.childNodes).filter(n => n instanceof HTMLElement) as HTMLElement[];
  ...
  it('edge: after reordering reactive array, slotIds reflect new sidecar order', async () => {
    ...
    const nodes1 = Array.from(container.childNodes).filter(n => n instanceof HTMLElement) as HTMLElement[];
    ...
    const nodes2 = Array.from(container.childNodes).filter(n => n instanceof HTMLElement) as HTMLElement[];

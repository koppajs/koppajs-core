...
describe("component - edge cases and negative tests", () => {
  ...
  describe("validateProp - regex validation", () => {
    ...
    it("ignores regex validation for non-string props", () => {
      ...
      expect(
        validateProp({ propName: "count", propValue: 42, props }),
      ).toBe(true);
    });
  ...
  describe("validateProp - edge cases", () => {
    it("handles null value", () => {
      ...
      expect(
        validateProp({ propName: "value", propValue: null, props }),
      ).toBe(false);
    });
    ...
    it("handles NaN for number type", () => {
      ...
      expect(
        validateProp({ propName: "count", propValue: NaN, props }),
      ).toBe(true);
    });
    ...
    it("distinguishes between array and object", () => {
      ...
      expect(
        validateProp({ propName: "items", propValue: [1, 2, 3], props: propsArray }),
      ).toBe(true);

      expect(
        validateProp({ propName: "items", propValue: { a: 1 }, props: propsArray }),
      ).toBe(false);

      expect(
        validateProp({ propName: "config", propValue: { a: 1 }, props: propsObject }),
      ).toBe(true);

      expect(
        validateProp({ propName: "config", propValue: [1, 2, 3], props: propsObject }),
      ).toBe(false);
    });
    ...
    it("validates function type", () => {
      ...
      expect(
        validateProp({ propName: "callback", propValue: "not a function", props }),
      ).toBe(false);
    });
    ...
    it("handles undefined type (unknown)", () => {
      ...
      expect(
        validateProp({ propName: "anything", propValue: 42, props }),
      ).toBe(true);
    });
  });

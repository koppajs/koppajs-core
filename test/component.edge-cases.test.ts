...
import { validateProp, processSlots, registerComponent } from '../src/component'
...
describe('component - edge cases and negative tests', () => {
  ...
  describe('validateProp - regex validation', () => {
    ...
    it('handles complex regex patterns', () => {
      ...
      expect(validateProp({ propName: 'phone', propValue: '+1-555-1234', props })).toBe(
        false,
      ) // This specific format might not match the regex

      expect(validateProp({ propName: 'phone', propValue: '15551234', props })).toBe(true) // Simple numeric format should work
    })
  })
  ...
  describe('validateProp - edge cases', () => {
    it('handles null value', () => {
      ...
      expect(validateProp({ propName: 'value', propValue: null, props })).toBe(false)
    })
    ...
    it('handles NaN for number type', () => {
      ...
      expect(validateProp({ propName: 'count', propValue: NaN, props })).toBe(true)
    })
    ...
    it('handles Infinity for number type', () => {
      ...
      expect(validateProp({ propName: 'count', propValue: Infinity, props })).toBe(true)
    })
    ...
    it('validates function type', () => {
      ...
      expect(validateProp({ propName: 'callback', propValue: () => {}, props })).toBe(
        true,
      )
    ...
    it('handles undefined type (unknown)', () => {
      ...
      expect(validateProp({ propName: 'anything', propValue: 'string', props })).toBe(
        true,
      )

      expect(validateProp({ propName: 'anything', propValue: 42, props })).toBe(true)
    })
  })
  ...
  describe('registerComponent - error handling', () => {
    ...
    it('handles component with valid script that returns object', async () => {
      ...
      expect(() => registerComponent('test-valid-script', componentSource)).not.toThrow()

...
describe('helper utilities - edge cases and negative tests', () => {
  describe('setValueByPath - negative cases', () => {
    it('throws error for missing intermediate property', () => {
      ...
      expect(() => setValueByPath(obj, 'a.x.y', 10)).toThrow('Missing segment "x"')
    })
    ...
    it('throws error when trying to write to read-only property', () => {
      ...
      expect(() => setValueByPath(obj, 'a.readOnly', 10)).toThrow('is read-only')
    })
    ...
    it('handles primitive value as target', () => {
      expect(() => setValueByPath(42, 'prop', 10)).toThrow('target must be an object')

      expect(() => setValueByPath(true, 'prop', 10)).toThrow('target must be an object')
    })
